...
func determineDecryptionAlgorithm(key crypto.PublicKey, opts crypto.DecrypterOpts) (types.EncryptionAlgorithmSpec, error) {
	...
	switch pub.Size() {
	default:
		return "", fmt.Errorf("awskms does not support RSA public key size %d", pub.Size())
	case 2048, 3072, 4096:
		switch rsaOpts.Hash {
		case crypto.SHA1:
		...
		case crypto.Hash(0):
			...
			// way to determine which algorithm to use, though, so this is optimistic decryption.
			return awsOaepSha256, nil
		default:
