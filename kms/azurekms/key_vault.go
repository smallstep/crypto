...
// HSM by default; "managedhsm" defines if a key will be generated on a Azure
...
//   - azurekms:name=key-name;vault=vault-name
//   - azurekms:name=key-name;vault=vault-name;managedhsm=true
//
...
var createCredentials = func(_ context.Context, opts apiv1.Options) (azcore.TokenCredential, error) {
	...
	if opts.URI != "" {
		...
		cloudConf, err := getCloudConfiguration(u.Get("environment"), u.GetBool("managedhsm"))
		if err != nil {
			return nil, err
		}
...
// work.
func getCloudConfiguration(cloudName string, managed bool) (cloudConfiguration, error) {
