...
//   - azurekms:vault=vault-name;environment=env-name;privatelink=true
...
// public cloud if not specified; "privatelink" defines if the private DNS zone
// name should be used; "hsm" defines if a key will be generated by an HSM by
// default.
//
...
func New(ctx context.Context, opts apiv1.Options) (*KeyVault, error) {
	...
	if opts.URI != "" {
		...
		if u.GetBool("privatelink") && cloudConf.PrivateLink != "" {
			defaults.DNSSuffix = cloudConf.PrivateLink
		}
	}
...
func getCloudConfiguration(cloudName string) (cloudConfiguration, error) {
	...
	case "USGOV", "AZUREUSGOVERNMENT", "AZUREUSGOVERNMENTCLOUD":
		return cloudConfiguration{
			...
			PrivateLink:   "privatelink.vaultcore.usgovcloudapi.net",
		}, nil
	case "CHINA", "AZURECHINACLOUD":
		return cloudConfiguration{
			Configuration: cloud.AzureChina,
			DNSSuffix:     "vault.azure.cn",
			PrivateLink:   "privatelink.vaultcore.azure.cn",
		}, nil
