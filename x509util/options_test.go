...
{{- end }}
	...
	tests := []struct {
		name    string
		args    args
		want    TOptions[*x509.CertificateRequest]
		wantErr bool
	}{
		...
		}, cr}, TOptions[*x509.CertificateRequest]{
	...
	"extKeyUsage": ["serverAuth", "clientAuth"]
		...
		{"leafRSA", args{DefaultLeafTemplate, TemplateData{
			SubjectKey: Subject{CommonName: "foo"},
			SANsKey:    []SubjectAlternativeName{{Type: "dns", Value: "foo.com"}},
		}, crRSA}, TOptions[*x509.CertificateRequest]{
			CertBuffer: bytes.NewBufferString(`{
	"subject": {"commonName":"foo"},
	"sans": [{"type":"dns","value":"foo.com"}],
	"keyUsage": ["keyEncipherment", "digitalSignature"],
	"extKeyUsage": ["serverAuth", "clientAuth"]
		...
		{"iid", args{DefaultIIDLeafTemplate, TemplateData{}, cr}, TOptions[*x509.CertificateRequest]{
			CertBuffer: bytes.NewBufferString(`{
	"subject": {"commonName": "foo"},
	"dnsNames": ["foo.com"],
	...
	"extKeyUsage": ["serverAuth", "clientAuth"]
		...
		{"iidEscape", args{DefaultIIDLeafTemplate, TemplateData{}, crQuotes}, TOptions[*x509.CertificateRequest]{
			CertBuffer: bytes.NewBufferString(`{
	"subject": {"commonName": "foo\"}"},
	"dnsNames": ["foo.com"],
	"emailAddresses": ["foo@foo.com"],
	...
	"extKeyUsage": ["serverAuth", "clientAuth"]
		...
		{"admin", args{DefaultAdminLeafTemplate, TemplateData{}, cr}, TOptions[*x509.CertificateRequest]{
			CertBuffer: bytes.NewBufferString(`{
	"subject": {"commonName":"foo"},
	"dnsNames": ["foo.com"],
	"emailAddresses": ["foo@foo.com"],
	...
	"extKeyUsage": ["serverAuth", "clientAuth"]
		...
		{"adminRSA", args{DefaultAdminLeafTemplate, TemplateData{}, crRSA}, TOptions[*x509.CertificateRequest]{
			CertBuffer: bytes.NewBufferString(`{
	"subject": {"commonName":"foo"},
	"dnsNames": ["foo.com"],
	"emailAddresses": ["foo@foo.com"],
	...
	"extensions": [
		...
		{"fail", args{`{{ fail "a message" }}`, TemplateData{}, cr}, TOptions[*x509.CertificateRequest]{}, true},
		{"error", args{`{{ mustHas 3 .Data }}`, TemplateData{
			"Data": 3,
		}, cr}, TOptions[*x509.CertificateRequest]{}, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var got TOptions[*x509.CertificateRequest]
...
func TestWithTemplateBase64(t *testing.T) {
	...
	"extKeyUsage": ["serverAuth", "clientAuth"]
		...
		{"badBase64", args{"foobar", TemplateData{}, cr}, TOptions[*x509.CertificateRequest]{}, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var got TOptions[*x509.CertificateRequest]
...
func TestWithTemplateFile(t *testing.T) {
	...
	tests := []struct {
		...
		{"example", args{"./testdata/example.tpl", data, cr}, TOptions[*x509.CertificateRequest]{
			CertBuffer: bytes.NewBufferString(`{
	"subject": {"commonName":"foo"},
	...
	"extKeyUsage": ["serverAuth", "clientAuth"]
		...
		{"exampleRSA3072", args{"./testdata/example.tpl", data, rsa3072}, TOptions[*x509.CertificateRequest]{
			CertBuffer: bytes.NewBufferString(`{
	"subject": {"commonName":"foo"},
	"sans": [{"type":"dns","value":"foo.com"},{"type":"email","value":"root@foo.com"},{"type":"ip","value":"127.0.0.1"},{"type":"uri","value":"uri:foo:bar"}],
	"emailAddresses": ["foo@foo.com"],
	...
	"extKeyUsage": ["serverAuth", "clientAuth"]
		...
		{"exampleRSA2048", args{"./testdata/example.tpl", data, rsa2048}, TOptions[*x509.CertificateRequest]{}, true},
		{"missing", args{"./testdata/missing.tpl", data, cr}, TOptions[*x509.CertificateRequest]{}, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var got TOptions[*x509.CertificateRequest]
